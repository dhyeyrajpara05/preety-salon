...
const { User, Product, Cart, Category, Order, Service, Package, Appointment, Customer, MembershipPlan, Staff, Invoice, Feedback, ProductReview, Inquiry, Notification, Payment } = require('./models.cjs');

const app = express();

// Razorpay Instance
console.log('Razorpay Key ID:', process.env.RAZORPAY_KEY_ID ? 'Loaded (' + process.env.RAZORPAY_KEY_ID.substring(0, 8) + '...)' : 'MISSING');
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        // Create new user with default profile picture using UI-Avatars
        const newUser = new User({
            userid,
            uname,
            email,
            password: hashedPassword,
            profilepic: `https://ui-avatars.com/api/?name=${encodeURIComponent(uname)}&background=random&color=fff`,
            gender,
            contact
        });
...
app.get('/api/users/status/:email', async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Server error check user status' });
    }
});
...
app.post('/api/razorpay/create-order', async (req, res) => {
    try {
        const { amount, currency, receipt, price } = req.body;
        const finalPrice = price || amount; // Support both names
        ...
        // Match user's demo requirement of saving payment record during order creation
        const payData = new Payment({
            orderid: order.id,
            price: finalPrice,
            amount: finalPrice,
            status: 'created'
...
app.post('/api/razorpay/verify-payment', async (req, res) => {
    try {
        ...
        const { razorpay_order_id, razorpay_payment_id, razorpay_signature, amount, userid, internalOrderId } = req.body;
