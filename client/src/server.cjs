...
// Membership Seeding Logic
async function seedMemberships() {
    try {
        const count = await MembershipPlan.countDocuments();
        // Clear and reseeding as requested to ensure "Gold, Platinum, Diamond" are exactly what's there
        await MembershipPlan.deleteMany({});
        
        const plans = [
            {
                planName: 'Gold Ritual',
                price: 4999,
                discount: 15,
                durationMonths: 12,
                benefits: ['15% off all services', '1 priority booking per month', 'Special birthday ritual treat'],
                isActive: true
            },
            {
                planName: 'Platinum Ritual',
                price: 11999,
                discount: 25,
                durationMonths: 12,
                benefits: ['25% off all services', '3 priority bookings per month', 'Quarterly complimentary hair spa', 'Priority Artisan selection'],
                isActive: true
            },
            {
                planName: 'Diamond Ritual',
                price: 24999,
                discount: 40,
                durationMonths: 12,
                benefits: ['40% off all services', 'Unlimited priority bookings', 'Monthly complimentary service (up to ₹2000)', 'Dedicated Lifestyle Manager', 'VIP Lounge Access'],
                isActive: true
            }
        ];
        
        await MembershipPlan.insertMany(plans);
        console.log('✨ Membership Rituals Seeded (Gold, Platinum, Diamond)');
    } catch (error) {
...
app.post('/api/products', upload.single('pimg'), async (req, res) => {
    try {
        const { pname, price, pdesc, quantity, pcategory } = req.body;
...
app.put('/api/products/:pid', upload.single('pimg'), async (req, res) => {
    try {
        const { pname, price, pdesc, quantity, pcategory } = req.body;
        const updateData = { pname, price: Number(price), pdesc, quantity: Number(quantity), pcategory };
...
app.post('/api/categories', async (req, res) => {
    try {
        ...
        const newCategory = new Category({ catid, catname, catdesc });
...
app.post('/api/services', upload.single('image'), async (req, res) => {
    try {
        ...
        // Items will be stringified JSON when sent via FormData
        ...
        const newService = new Service({ sid, title, subtitle, gender, items, image });
...
// Update a service
app.put('/api/services/:sid', upload.single('image'), async (req, res) => {
    try {
        let { title, subtitle, gender, items } = req.body;
